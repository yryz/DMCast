...
function SetPrivilege(const Privilege: PChar): boolean; //SeShutdownPrivilege 关机权限  SeDebugPrivilege 调试权限
function RegDelValue(const Key, Vname: PChar): boolean; //删除注册表值
function RegReadStr(const Key, Vname: PChar): string; //读注册表 str
function RegReadInt(const Key, Vname: PChar): DWORD; //读注册表Integer
function RegWriteStr(const Key, Vname, Value: PChar): boolean; //写STR
function RegWriteInt(const Key, Vname: PChar; const Value: Integer): boolean; //写DWORD
